...
- 보험료 계산 탭: 4개사 실제 견적 기반 간이 계산기(연령대 + 플랜만 입력받음).
  여행 일수는 플래너(page2)에서 정한 trip_context["plan"]의 날짜로 자동 계산한다.
...
def premium():
    ...
    for age_group in insurance_service.PREMIUM_AGE_GROUPS:
        # 폼 필드명은 연령대 문자열을 그대로 쓰기 애매해서 인덱스로 만든다 (age_count_0, age_count_1, ...)
