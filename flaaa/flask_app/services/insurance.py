...
보험료 계산(PREMIUM_*)은 4개사 다이렉트 사이트에서 동일 조건(2026.08.06~08, 2일)으로
직접 뽑은 실제 견적을 기준값으로 삼은 간이 시뮬레이션이다. Streamlit 쪽
(views/page4_insurance.py)과 완전히 동일한 로직을 그대로 옮겨왔다.
"""
...
# ---------------------------------------------------------------------------
# 보험료 계산 데이터 (Streamlit views/page4_insurance.py와 동일)
# ---------------------------------------------------------------------------
PREMIUM_BASE_RATES = {
...
def calculate_premiums(days: int, age_counts: dict[str, int], plan: str) -> list[dict]:
    """4개사 예상 보험료를 계산한다. age_counts는 {연령대: 인원수} - 동행자마다
    연령대가 다를 수 있어서 연령대별로 각자의 배율을 적용해 합산한다."""
    rows = []
