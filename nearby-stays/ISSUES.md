...
## 0004. 다크 테마에서 카드 안 숙소 이름이 흰 배경에 흰 글씨로 안 보이고, 막대그래프 라벨이 세로로 표시됨

- **서비스 / 단계**: nearby-stays, 실사용 테스트 (사용자 브라우저가 다크 테마)
- **증상**:
  1. 숙소 카드(`.stay-card`)는 항상 밝은 크림색 배경으로 고정해뒀는데, 숙소 이름
     텍스트(`.stay-name`)에는 글자색을 지정하지 않아서 Streamlit 다크 테마의 기본
     흰색 글자색을 그대로 상속받아 "흰 배경에 흰 글씨"로 안 보이는 문제 발생.
  2. `st.bar_chart`로 그린 "숙소 유형별 분포" 그래프의 x축 라벨(호텔/모텔/게스트하우스 등)이
     자동으로 세로로 회전되어 표시되어 읽기 어려움.
- **원인**:
  1. 카드 배경색은 라이트/다크 테마와 무관하게 항상 밝게 고정했지만, 텍스트 색은
     명시적으로 지정하지 않아 페이지 전체 테마(다크)의 기본 글자색을 그대로 물려받음.
  2. `st.bar_chart`는 내부적으로 Vega-Lite를 쓰지만 세부 축 설정(라벨 각도 등)을
     커스터마이징할 수 있는 파라미터를 제공하지 않아, 라벨이 길면 자동으로 회전시킴.
- **해결**:
  1. `.stay-name`, `.stay-address`에 명시적인 진한 글자색(`#1a1a1a`, `#6b6b6b`)을
     지정해 테마와 무관하게 항상 카드 배경과 대비되도록 고정.
  2. `st.bar_chart` 대신 `altair`로 직접 차트를 구성해 `axis=alt.Axis(labelAngle=0)`으로
     x축 라벨 각도를 0도(가로)로 고정.
- **교훈**: 카드처럼 배경색을 고정한 커스텀 UI를 만들 때는 **배경뿐 아니라 텍스트 색도
  함께 명시적으로 고정**해야 한다. Streamlit 기본 컴포넌트(`st.bar_chart` 등)로
  세밀한 스타일 제어가 안 될 때는 한 단계 아래의 `st.altair_chart`로 내려가서 직접
  인코딩을 제어하면 된다.
